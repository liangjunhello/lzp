...
class ClipFixed(nn.Module):
    ...
    def get_llm_text_features(self,dataset,n_shot):
        ...
        fold_path=os.path.join(fold_path, res)
        ...
        clip_weights_permuted = clip_weights.permute(1, 0, 2)
        with torch.no_grad():
            a=self.fixed_local_feat
        llm_local_text=torch.cat((clip_weights_permuted,a),dim=1)

        self.llm_local_text = llm_local_text
        return llm_local_text


    def get_simple_text_features(self,dataset,n_shot):
        text_feat = []
        for name in self.classnames:
            text_prompts = [template.format(name) for template in self.text_prompt]
            texts = tokenize(text_prompts).cuda()
            ...
            replicated_embeddings = class_embeddings.repeat(17, 1)
            text_feat.append(replicated_embeddings)

        self.simple_text = torch.stack(text_feat, dim=0).cuda()
