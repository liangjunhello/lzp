...
class ClipTestTimeTuning(nn.Module):
    ...
    def get_text_features(self):
        text_features = []
        prompts = self.prompt_learner(with_std=True)
    ...
    def inference(self, image):
        ...
        if self.return_local_feat:
            ...
            image_features = image_features_vanilla[:,0,:]
        ...
        if self.first_flag:
            self.image_feature_memory = torch.zeros(text_features.shape[0], self.memory_size, image_features.shape[1])
            self.image_prediction_mem = torch.zeros(text_features.shape[0], self.memory_size, self.num_class)
            self.image_entropy_mem = torch.zeros(text_features.shape[0], self.memory_size)
            ...
            self.local_entropy_mem = torch.zeros(text_features.shape[0], self.memory_size)
            self.local_feature_count = torch.zeros(text_features.shape[0], 1).long()
            self.syn_sample = self.text_features.detach()
