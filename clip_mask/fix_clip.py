...
from clip_mask.clip import load, tokenize
...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, batch_size=None, n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False):
        ...
        self.ctx_std = nn.Parameter(torch.ones(ctx_vectors.shape) * 1e-4)
    ...
    def reset(self):
        ...
        self.ctx = self.ctx.copy_(ctx_vectors)
        if self.learned_cls:
...
class ClipFixed(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, text_prompt='tip'):
    ...
    def reset_classnames(self, classnames, test_sets):
        ...
        if self.text_prompt_type == 'simple':
            self.text_prompt = simple_imagenet_template
        elif self.text_prompt_type =='tip':
            if len(test_sets)>1:
                self.text_prompt = ID_to_prompts[test_sets.lower()]
            else:
                self.text_prompt = tip_imagenet_templates
    ...
    def get_text_features(self):
        ...
        for name in self.classnames:
            text_prompts = [template.format(name) for template in self.text_prompt]
            if self.text_prompt_type =='tip_cupl':
                text_prompts += self.cupl_prompts[name]
            texts = tokenize(text_prompts).cuda()
            ...
            text_feat.append(class_embedding_mean)
            one_hot_target = torch.zeros(self.n_cls).to(class_embedding_mean.device)
            one_hot_target[count] = 1
            text_label.append(one_hot_target)
            count = count + 1
        self.text_feat = torch.stack(text_feat, dim=0).cuda()
        self.text_label = torch.stack(text_label, dim=0).cuda()

        self.text_feat_full = self.text_feat
        self.fixed_global_feat = self.text_feat.clone().unsqueeze(1)
        self.fixed_local_feat = self.text_feat.clone().unsqueeze(1)
        self.fixed_global_feat_vanilla = self.text_feat.clone().unsqueeze(1)
        self.fixed_local_feat_vanilla = self.text_feat.clone().unsqueeze(1)
        ...
        if self.first_flag:
            ...
            self.image_prediction_mem = torch.zeros(self.n_cls, self.memory_size, self.n_cls).to(self.text_feat.device)
            self.image_entropy_mem = torch.zeros(self.n_cls, self.memory_size).to(self.text_feat.device)
            ...
            self.local_prediction_mem = torch.zeros(self.n_cls, self.memory_size, self.n_cls).to(self.text_feat.device)
            self.local_entropy_mem = torch.zeros(self.n_cls, self.memory_size).to(self.text_feat.device)
    ...
    def get_image_attn(self, image):

        image_features,attn_weight_list = self.clip.encode_image(image,attn_mask=1,count=1)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        image_features_local = image_features[:,1:,:]

        self.image_features_local = image_features_local
        self.attn_weight_list=attn_weight_list
...
def get_attn_clip(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False, memory_size=10, text_prompt='tip'):

    model = ClipFixed(device, classnames, None, arch=clip_arch, n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls,text_prompt=text_prompt)
