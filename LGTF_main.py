...
def important_channel_indice(args, model, only_use_txt=True):
    if only_use_txt or args.shot  == 0:
        feats = model.text_feat.unsqueeze(1)
    else:
        feats = model.fixed_global_feat_vanilla
...
def select_confident_samples(prob, top):
    ...
    idx_confused = torch.argsort(batch_entropy, descending=False)[int(batch_entropy.size()[0] * top):]
...
class Localpart(nn.Module):
    def __init__(self,args=None, beta=5.5, feat_dim=1024, class_num=1000, mapping='bias'):
        ...
        self._attn_layers = [9, 10, 11]
    
    def set_attention_layers(self, layers):
        """Set which attention layers to use for feature extraction"""
        self._attn_layers = layers
    
    def get_attention_layers(self):
        """Get current attention layers configuration"""
        return self._attn_layers
    def get_text_bias(self):
        return self.dmnet.get_text_bias().detach()
    ...
    def get_image_pred_attn_global_version2(self, model_clip,model_attn,return_full=False, return_logit=False):
        ...
        img_feat = model_clip.image_features_global
        ...
        if N < 17:
            ...
            random_image_feats = memorized_image_feat_V[:, random_indices, :]

            memorized_image_feat_V = torch.cat([memorized_image_feat_V, random_image_feats], dim=1)
        else:
        ...
        with torch.no_grad():
            ...
            attn_vote = torch.stack(attn_weight, dim=0).permute(1, 0, 2, 3)
            thres0 = attn_vote.reshape(batch_size, 12, -1).median(dim=-1, keepdim=True).values
            thres0=thres0.unsqueeze(-1)
            thres0 = thres0.expand(-1, -1,attn_vote.shape[2], attn_vote.shape[3])
            attn_weight = torch.stack(attn_weight, dim=0)[self._attn_layers]
            attn_cnt = (attn_vote > thres0).float()
            attn_cnt = torch.sum(attn_cnt, dim=1)
            attn_cnt = (attn_cnt >= 4).float()
            attn_weight = torch.mean(attn_weight, dim=0)
            attn_weight=attn_weight*attn_cnt
        logits_list = []
        logit_local_scale = model_clip.logit_scale.exp()
        llm_local_Key=llm_local_Key+self.local_llm_bias_key.unsqueeze(1)
        for i in range(batch_size):
            image_features_i = img_feat_loacl_mappling[i]
            attn_weight_i = attn_weight[i]
            logits = logit_local_scale*llm_local_Key @ image_features_i.T
            logits = logits.softmax(dim=-1)
            logits = torch.matmul(logits,attn_weight_i)

            logits=torch.sum(logits,dim=2,keepdim=True)
        ...
        adaptive_image_feat = memorized_image_feat_V.transpose(1,2) @ similarity_matrix
        adaptive_image_feat = adaptive_image_feat / adaptive_image_feat.norm(dim=1, keepdim=True)
        adaptive_image_feat = adaptive_image_feat.transpose(0,2).transpose(1,2)

        logit_scale = model_clip.logit_scale.exp()
        adaptive_image_feat = adaptive_image_feat + self.local_ffn_bias.unsqueeze(0)
        adaptive_image_feat = adaptive_image_feat / adaptive_image_feat.norm(dim=-1, keepdim=True)
        logits = logit_scale * adaptive_image_feat[..., self.args.indice] @ img_feat[..., self.args.indice].unsqueeze(-1)
        if return_logit:
            return logits[:,:,0]
        else:
    ...
    def get_text_prediction(self, model, return_full=True, return_logit=False):
        ...
        img_text_logit = logit_scale * model.image_features_global @ text_feat.t()
        if return_full:
            pass
        else:
                ...
                for lr in lr_list:
                    for epoch in epoch_list:
                        ...
                        if args.num_important_channel != 0:
                            ...
                            important_indice = torch.arange(model_clip.text_feat.shape[1]).to(model_clip.text_feat.device)
                            args.indice = important_indice
...
def entropy(outputs):
    logits = outputs - outputs.logsumexp(dim=-1, keepdim=True)
    avg_logits = logits.logsumexp(dim=0, keepdim=True) - np.log(logits.shape[0])
    min_real = torch.finfo(avg_logits.dtype).min
    avg_logits = torch.clamp(avg_logits, min=min_real)
...
def init_image_memory(train_loader, model,model_version, args):
    ...
    memorized_image_global_feat = []
    memorized_image_local_feat = []
    memorized_image_global_feat_vanilla = []
    memorized_image_local_feat_vanilla = []
    memorized_labels = []
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        text_features = model.text_feat[target]
        selected_image_features_local = model.image_features_local
        cos_sim = (selected_image_features_local * text_features).sum(-1)
        weight_prob = (cos_sim * 100).softmax(-1)
        attented_feat = (weight_prob.unsqueeze(-1) * selected_image_features_local).sum(1)
        attented_feat = attented_feat / attented_feat.norm(dim=-1, keepdim=True)
        memorized_image_global_feat[target].append(image_features_global)
        memorized_image_local_feat[target].append(attented_feat)
        memorized_image_global_feat_vanilla[target].append(image_features_global[:1])
        memorized_image_local_feat_vanilla[target].append(attented_feat[:1])
        one_hot_target = torch.zeros(1, model.n_cls).to(target.device)
        one_hot_target[0, target] = 1
        memorized_labels[target].append(one_hot_target)

    for i in range(model.n_cls):
        memorized_image_global_feat[i] = torch.cat(memorized_image_global_feat[i], dim=0).unsqueeze(0)
        memorized_image_local_feat[i] = torch.cat(memorized_image_local_feat[i], dim=0).unsqueeze(0)
        memorized_image_global_feat_vanilla[i] = torch.cat(memorized_image_global_feat_vanilla[i], dim=0).unsqueeze(0)
    ...
    memorized_image_global_feat = torch.cat(memorized_image_global_feat, dim=0)
    memorized_image_local_feat = torch.cat(memorized_image_local_feat, dim=0)
    memorized_image_global_feat_vanilla = torch.cat(memorized_image_global_feat_vanilla, dim=0)
    ...
    model.fewshot_image_global_feat = memorized_image_global_feat
    model.fewshot_image_local_feat = memorized_image_local_feat
    model.fewshot_image_global_feat_vanilla = memorized_image_global_feat_vanilla
    model.fewshot_image_local_feat_vanilla = memorized_image_local_feat_vanilla
    model.fewshot_label = memorized_labels

    model.fixed_global_feat_vanilla = torch.cat((model.fixed_global_feat, memorized_image_global_feat_vanilla), dim=1)
    model.fixed_local_feat_vanilla = torch.cat((model.fixed_local_feat, memorized_image_local_feat_vanilla), dim=1)

    model.fixed_global_feat = torch.cat((model.fixed_global_feat, memorized_image_global_feat), dim=1)
    model.fixed_local_feat = torch.cat((model.fixed_local_feat, memorized_image_local_feat), dim=1)
...
def direct_inference(val_loader, model_clip,model_attn, args):
    ...
    if args.n_shot and args.ft:
        ...
        preprocess = transforms.Compose([
            transforms.ToTensor(),
            normalize])
        data_transform_aug = StrongAugmenter(base_transform, preprocess, augmix=len(args.set_id) > 1)
        ...
        train_loader_ft = torch.utils.data.DataLoader(
            train_dataset_mem,
            batch_size=128 if training_size > 128 else training_size, shuffle=True,
        ...
        for train_idx in range(epoch):
            ...
            for i, (images, target) in enumerate(train_loader_ft):
                ...
                if args.position == 'output' or args.position == 'all':
                    ...
                    localpart.init_pred = text_logit
    ...
    torch.save(localpart.state_dict(),'flower102_16shot.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_important_channel', type=str, default='0')
